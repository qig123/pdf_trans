...
alignment, 236, 319, 354, 356, 357, 368,
C 99ff
...
Arabic numerals, 43
architecture, 217ff; C 60ff. See also CISC;
...
index type, 359
memory layout, 368ff. See also
in context-free grammar, 52ff, 80
predict-predict conﬂict, 89
shift-reduce conﬂict, 94
AMD Corp., 22; C 81
AMD64 architecture, C 99
American National Standards Institute
RISC; speciﬁc architectures
ARM as example, C 80ff
instruction set, C 70ff
multiprocessors, 631ff
processor implementation and, 824;
(ANSI), 211, 771, 861–863, 866,
868
C 75ff
x86 as example, C 80ff
arguments, 32, 35, 120, 411. See also
...
evaluation order, 240ff, 282ff, 567ff,
...
C 13
ﬁnite, 55, 94, 103, 746, 749; C 13ff.
...
attribute ﬂow, 180, 187ff, 190ff. See also
714ff
...
goto statement, 7
scope rules, 126
basic block, 776, 829; C 93, C 276, C 297,
overloading, 147ff
in Prolog, 599, 616
...
binding rules, 126, 152ff, 155. See also
...
bag of tasks model of concurrency, 635,
closure, subroutine
deep, 126, 152ff
...
Library), 839
Beck, Leland L., 806
Bell Telephone Laboratories, 9, 718, 846,
861, 865, 868; C 37
Bell, C. Gordon, C 105
Bellard, Fabrice, 854
Bernstein, Robert L., 294
best ﬁt algorithm, 122
...
blocking (of process/thread), 628, 636,
753
...
307; C 86
binding, 3, 116ff, 118. See also binding
...
computed target, 829
conditional, 254, 263ff; C 72ff, C 75,
...
pipelining and, C 90
in position-independent code, C 280,
C 206, C 323ff, C 333
...
restrict qualiﬁer, 146
run-time system, 807
scope rules, 125, 132, 134
separate compilation, 137, 165, 800
initialization, 240, 475, 496ff, 502
operator functions, 148, 225, 429,
478; C 157
overloading, 148ff
parameter passing, 423, 427ff, 435ff.
325, 348, 358, 430, 432, 501, 507,
514, 538, 679, 685, 698; C 122,
...
references, 118, 231, 361, 427ff, 467,
nested, 490
coercion, 322
comments, nested, 55
...
run-time type identifcation, 853
scope resolution, 128
scope rules, 132, 193
streams, C 156ff
strings, 376
...
memory layout, 357
subroutine calling sequence, 418;
decltype, 326
...
type checking, 513
type extension, 491
close operation, for ﬁles, C 150
...
combinator, ﬁxed-point, 590; C 219,
621
...
just-in-time, 23, 421, 512, 776, 810,
global, C 315ff
communication, 427, 635ff; C 343. See
vs Java Virtual Machine, C 287ff
just-in-time compilation, C 287
language interoperability, 820, 821;
...
type safety, C 287
unsafe code, C 288
veriﬁcation, C 288, C 292
compare_and_swap (CAS) instruction,
vs validation, C 294
Virtual Execution System (VES),
655ff, 682
compilation unit, 797ff; C 40. See also
separate compilation
compiled languages, 18
compilers and compilation, 9, 11, 15,
C 286
...
26ff, 70, 72, 77, 102, 179, 181ff,
198, 217. See also scanners and
...
of functional languages, 279, 582ff,
arrays, 367
...
one-pass, 193ff, 209
phases, 26ff, 37, 776ff, 780. See also
...
throw), 248, 249, 291, 443
nested subroutines, 127
objects. See CLOS
parameter passing, 435ff
programming environment, 9, 25ff,
684
history, 6
IFs (intermediate forms), 780ff;
CISC
...
unwind-protect,447
Worse Is Better, 764
common preﬁx in CFG, 74, 79ff
common subexpression elimination,
C 77, C 88ff, C 98, C 178
interaction with language design,
xxvff, 8, 166, 285, 286, 403ff, 462,
689; C 241. See also Appendix B
assignment operators, 235
xxix
...
race condition, 626, 639, 640, 650ff,
style, 250ff, 279, 291, 295, 448,
469, 547
...
constructor function, 240ff, 297, 382,
systems, C 129, C 135
control abstraction, 115, 221, 223, 411ff,
422. See also subroutine;
...
condition codes, 834; C 62, C 72, C 102,
475, 495ff. See also initialization
choosing, 495
copy constructor, 430, 499
move constructor, 430, 501
overloaded, 475
container class, 268, 323ff, 333, 484, 530;
C 329
...
control ﬂow analysis, C 325
control ﬂow graph, 776, 829; C 298
predict-predict, 89
shift-reduce, 94
...
currying of functions, 328, 577ff, 621,
Datalog, 621
...
Cooper, Keith D., 41, 215, 806; C 106,
865; C 213, C 221ff
cut (!), in Prolog, 604
CWI Amsterdam, 720
...
dangling reference, 118, 124, 156, 184,
Synthesizer Generator, 214, 215
coroutine, 450ff, 469, 648, 809
class, 476
executable, 763
vs deﬁnition, 133, 316; C 37ff
of loop index, 266
order, 130ff
context block, 455
...
data abstraction, 115, 135ff, 144, 221,
456; C 187ff
...
declarative language, 11, 613, 617, 620;
transfer, 454ff
Courcelle, Bruno, 215
...
data ﬂow, 685
data ﬂow analysis, 820; C 225, C 294,
C 129, C 135
Cox, Brad J., 865
Cray, Inc., 634, 697
...
deﬁnite assignment, 33, 184, 239, 347,
820; C 287, C 294, C 350
deﬁnition vs declaration, 133, 316; C 37ff
Dekker, Theodorus (Dirk) Jozef, 653
delay slot, C 91ff, C 303
delayed branch. See branch instruction,
Processes), 182, 863, 866. See
also Go; Occam
...
forward, C 317
data formats in memory, C 63ff. See also
delayed
...
C 259
DTD (document type deﬁnition), 738;
anti- (read-write), 685; C 92, C 329
ﬂow (write-read), C 92, C 329
...
495, 504ff. See also ﬁnalization
...
C 319
Efﬁceon (Transmeta) architecture, 830
...
endian-ness, 319, 344, 357; C 64ff,
for message passing, C 242ff
supported by reﬂection, 837
...
ﬁlename expansion in bash, 706ff
ﬁles, 401ff; C 148ff. See also I/O
...
commercial, 724
extent, of object deﬁnition, 156, 646;
C 191
and Ada tasks, 642
...
ﬁnite automaton, 55, 94, 103, 746, 749;
F
F#, 326, 550, 863. See also ML; OCaml
613
existential quantiﬁer, 615; C 226ff
exit, from loop, 265
...
also IA-64 (Itanium)
architecture
exponent of ﬂoating-point number, C 67
...
ﬁxed point of function, 581; C 218,
in Ada, C 153
in C, C 154ff
in C++, C 156ff
in Emacs Lisp, 726
...
C 225, C 316ff, C 321
ﬁxed-format language, 48
ﬁxed-point combinator, C 219ff, C 223
ﬁxed-point type, 305
Flash, 724
Flavors, 530
Fleck, Arthur C., 468
flex. See lex
ﬂoating-point, C 67ff, C 76, C 90
self-representation
522
Fortress, 698
...
higher-order functions, 439, 538,
...
gcc (GNU compiler collection), 6, 75,
576ff
...
iterators
GENERIC (gcc IF), 782
generic subroutine or module, 286, 319,
...
gap, ﬂoating-point, C 68
garbage collection, 119, 124, 156, 378,
lambda expression; C#,
...
and reﬂection, 839, 841; C 127
reiﬁcation, 841; C 128, C 291
...
and ﬁnalization of objects, 496, 504ff
in functional programming, 536,
function
functional languages and programming,
11, 535ff. See also Common
Lisp; Erlang; Haskell; lambda
calculus; Lisp; ML; OCaml;
recursion; Scheme; Sisal
aggregates, 538ff
...
ﬁrst-class subroutines, 155ff, 538ff
C 312ff
global variable, 126
Index
929
...
Haskell, 326, 537, 550, 590, 864. See also
C 111, C 218ff, C 273ff
...
heap, 119, 122ff, 792, 807. See also
arrays, 362
...
guarded commands, C 110ff, C 117,
C 245
...
C 344, C 355
graphical user interface (GUI), 456, 523,
...
Hewlett-Packard Corp., 211, 523, 633,
41, 182, 215, 246, 294, 295,
348ff, 669, 861, 866
Hoare, Graydon, 867
...
endian-ness, C 64
memory model, 661
microprogramming, C 76
IBM 704 architecture, 119, 368, 399
IBM Corp., 9, 861, 863, 866
782, 830, 831
hexadecimal notation, C 65
Heymans, F., 698
High Performance Fortran (HPF), 640,
multiprocessors, 633
...
identiﬁer, 32, 45. See also names
...
immediate error detection problem, C 3,
536ff
of HTML, 736
of Java, 637, 812
of Lisp, 6, 399, 468
...
canonical, 703
compilation and interpretation, 17ff,
I
IA-32 EL binary translator, 831
IA-64 (Itanium) architecture, 633, 830,
831, 834; C 99, C 104
...
import
C 273ff, C 313
level of machine dependence, 780;
...
inherited
initialization, 238ff. See also assignment;
524, 646, 824, 825; C 97. See
also macros
in-order execution, 241; C 90
incremental compilation. See compilers
Standardization (ISO), 621,
736, 810, 861, 865, 866; C 291
...
index, of loop, 262ff, 285. See also
languages, 706ff, 748ff
...
of Scheme, 539
and scripting, 702, 712, 718, 724, 731,
C 354
instantiation
implicit, of generic subroutine, 338
of variables in Prolog, 593, 595, 599,
See also lazy evaluation
inﬁx notation, 148, 211, 225ff, 240
734, 754, 764
tree traversal, 33
vs virtual machines, 810
interprocedural code improvement. See
616
instruction scheduling, 419, 780, 795,
in ML, 226, 422
in Prolog, 595, 607
in R, 226, 718
in Smalltalk, 226
...
as a data format, C 63
useless, C 303, C 321
Intel 64 architecture, C 99. See x86-64
implementation, 510ff; C 194ff,
of loop, 290, 295
of monitor, 671
I/O (input/output), 401ff, 807; C 148ff.
C 209ff
mix-in, 509, 516ff
modules and, 486ff
multiple, 472, 521ff; C 194ff
architecture
Intel Corp., 22, 834; C 81, C 99. See also
See also ﬁles; event handling;
graphical user interface
interactive, C 148ff
x86 architecture; IA-64
...
streams, 571ff
iPhone, 9, 418
ISA (instruction set architecture), 810;
bytecode
capitalization, 47
casts, 323, 511
classes, 141
...
polymorphism, 302
precedence levels, 227
Real Time Speciﬁcation for, 390
recursive types, 7, 379
reﬂection, 611, 837, 838ff
regular expressions, 743
safety, 241
scope rules, 132, 193
scoped memory, 390
...
J
Järvi, Jaakko, 178
Java, 7, 8, 12, 864. See also Java Virtual
Machine
...
Jordan, Mick, 865
Joy, William, 705
JScript, 736, 864; C 286. See also
instruction set, 817
...
C 287ff
constant pool, 813ff
and languages other than Java, 813,
for goto, 246
nonlocal, 247
...
Kernighan, Brian W., 41, 295, 714,
arrays and hashes, 755
ﬁrst-class subroutines, 739
...
keywords, 45, 47, 63. See also speciﬁc
keywords and languages
contextual, 46, 64
predeﬁned names and, 64, 175
Kinnersley, Bill, 859
...
Landin, Peter, 178
language, 45. See also speciﬁc languages
757
and XSLT, C 262
Jazayeri, Mehdi, 215
JCL, 700
...
design, 7ff
844
job control in shell languages,
binding time and, 116
extension, 16, 724ff
families, 11ff
formal, 103; C 14
free vs ﬁxed format, 47
functional, 11, 535ff
707
...
Index
935
resolution and uniﬁcation, 595ff
...
machine-speciﬁc code improvement, 27;
C 299ff
...
message passing, 687; C 235ff. See also
Manson, Jeremy, 696, 698
mantissa of ﬂoating-point number, C 67
Maple, 717, 756, 771
...
Pages; ActiveX; C#; COM;
Common Language
Infrastructure; DCOM; DDE;
F#; Internet Explorer; .NET;
OLE; PowerShell; Windows
operating system; Visual Basic;
Visual Studio
compilers, 826
Microsoft Word, 783
mid-test loop, 276ff
middle end, of a compiler, 27, 181, 775ff;
MPI; PVM; receive
...
memory, C 61. See also alignment;
...
mix-in
“mixﬁx” notation, 226, 422; C 107,
bypassing, 660
C++11, 662, 679
conﬂicting operations, 675, 680
Java, 662, 678ff, 696
sequential consistency, 659ff
temporal loop, 660ff
Mercury, 597, 621
C 204
ML, 11, 621, 865. See also F#; Haskell;
Miranda; OCaml; SML
aggregates, 238, 400
arrays, 556
assignment, 383, 589
...
Microsoft Corp., 9, 348, 530, 550, 861,
...
Modula-3, 865
semantics, 671ff
signals as hints and absolutes, 671ff,
currying, 578
equality testing, 340
exception handling, 441, 446
ﬁrst-class functions, 432, 577
imperative features, 383
inﬁx notation, 226, 422
lists, 398ff, 555
nested subroutines, 127
...
MPI (message passing interface), 634,
arrays, 360
case of letters, 47
...
Language), C 287. See also
Common Language
Infrastructure, Common
Intermediate Language
Muchnick, Steven S., 806; C 106, C 355ff
multicore processor, 624, 625, 632; C 60,
521
module, 135ff, 486ff
and classes, 139ff
closed scope, 138
...
multilingual character set, 306, 349. See
mod operator, 345
...
also Unicode
Multilisp, 684, 695
multipass compiler, 207. See also
goto statement, 246
...
multiprocessor, 533
architecture, 631ff
cache coherence, 632, 633ff, 654, 691;
...
predeﬁned, 47, 64, 128, 175
qualiﬁed, 128, 723, 838; C 39ff, C 48
...
generics and, 481ff
initialization and ﬁnalization, 495ff
modules and, 139ff
...
overﬂow, in binary arithmetic, 33, 38,
references
ambiguity of, 425ff
by result, 427
by sharing, 425
...
of constructors, 475
deﬁnition, 145
overriding of method deﬁnitions, 141ff,
436ff; C 149
...
coercion, 751
context, use of, 703, 719, 751ff, 756ff;
attributes and, 200
standardization, 8
...
Space) languages, 698
pH (language), 537, 546, 582, 683, 697,
subroutine or module
implicit, 327, 329, 332, 535
in Scheme, 541
subtype, 302, 505. See also methods
864
phase of compilation, 26ff, 37, 776ff,
780; C 299ff. See also scanning;
parsing; semantic analysis; code
improvement; code generation
φ (phi) function, in SSA form, C 313ff
PHP, 12, 700, 731ff, 866
and method binding, dynamic
type systems and, 302ff
...
position-independent code (PIC), C 279,
724, 734
...
syntax, 378ff
Polak, W., 469
Polish postﬁx notation, 211ff. See also
C 280ff
...
postﬁx notation, 211ff, 225, 287
Postscript, 24, 211, 226, 706ff, 782, 783,
757
and XSLT, C 262
phrase-level recovery from syntax errors,
102, 113; C 2ff, C 10ff
PIC (position-independent code), C 279,
867; C 151
Pouzin, Louis, 705
Power architecture, 264, 633, 780, 829;
C 280ff
...
pipelining, C 75, C 89ff, C 100. See also
C 99ff, C 105ff
addressing modes, C 72
...
threads and, 635, 647
processor, 632. See also architecture;
PowerShell, 700, 724, 731
pragma (compiler directive), 67ff, 419,
444, 638, 842; C 160. See also
annotations, in Java; attribute,
in C++ and C#
pre-test loop, 275
precedence, 287, 422
microprocessor;
...
processor
compiling for modern processors, 36;
...
deﬁnition, 52
in lambda calculus, C 214
of operators, 163, 224, 226ff, 240,
C 229
parallelization, 686
parentheses, 593
queries, 594, 598
...
predeﬁned names, 47, 128
Proebsting’s Law, C 354
prof, 848
proﬁle-driven code improvement, 824ff
proﬁler, 24, 834, 848ff
keywords and, 64, 175
...
type checking, 593
uniﬁcation, 331, 593, 595ff, 599
variables, 593, 595
prologue of subroutine, 120, 414
promise, 283
pronunciation, of language names, 7
proof, constructive vs nonconstructive,
...
prefetching, 184
preﬁx argument in Emacs Lisp, 726
preﬁx notation, 225, 287, 607. See also
537, 617
proof-carrying code, 853
proper list. See list, proper
property mechanism in C#, 477
proposition, in predicate calculus, 612;
Cambridge Polish notation
preﬁx property, of CFL, C 20
...
virtualization system, 831, 854
quadruples, 781
qualiﬁed name, 128, 723, 838; C 39ff,
C 48
quantiﬁer, 612, 615ff; C 226ff
quasiparallelism, 624
query
anti-
reader–writer locks, 655, 675, 678, 691
ready list, 649ff, 653, 663ff, 690; C 251
Real Time Speciﬁcation for Java, 390
...
recursion, 45, 223, 277ff. See also tail
recursion
...
reﬁnement of abstractions, 473
reﬂection, 144, 611, 791, 810, 837ff; C 44
machines), C 71
...
Index
rule, in Prolog, 593
run-time system, 807ff. See also
Rubin, Frank, 295
...
C 77ff. See also Alpha
architecture; ARM architecture;
MIPS architecture; PA-RISC
architecture; Power
architecture; SPARC
architecture
delayed branch, C 90
...
lambda expressions and procs, 159,
S
250, 273, 294, 538
line breaks, 722
method binding, 507
...
Rosetta (Apple) binary translator, 829,
memory
S-expression, 547; C 274
SAC (Single Assignment C), 537, 582,
throw), 248, 249, 443
multiway assignment, 236, 704
nested blocks, 739, 768
nested subroutines, lack of, 739
numeric types, 719, 753
objects, 141, 472, 522, 704, 722, 757ff,
868
safety vs performance, 182, 241, 285;
C 325
Saltz, Joel, C 354
...
row-major layout (of arrays), 368ff;
futures, 685
garbage collection, 378, 390
generics, 333
C 337ff
...
947
hole in, 128, 132
implementation, 128, 144, 412;
lazy evaluation, 586
let construct, 132, 542
lists, 543ff
macros, 164, 569, 571
scalar type. See type, scalar
scanner generator, 56ff, 62, 104. See also
lex
scanners and scanning, 3, 27, 28ff, 44,
C 26ff. See also display; static link
...
54ff
ad hoc, 55
with explicit ﬁnite automaton, 55,
map function, 576
nested subroutines, 127
numbers and numeric functions, 305,
61ff
lexical errors, 65ff
longest possible token rule, 55, 64ff
nontrivial preﬁx problem, 64
table-driven, 65ff
scatter operation, C 244, C 249, C 251
scheduler-based synchronization, 636,
544, 719, 753
...
S-expressions, 547
scope rules, 125, 132, 134, 142, 412,
classes, 139ff, 488ff
declaration order, 130ff
declarations and deﬁnitions, 133ff
modules, 135ff, 486ff
nested blocks, 134ff, 542ff; C 26
nested subroutines, 127ff; C 26
665ff
scheduling, 648, 663ff; C 251. See also
...
543, 739
searching, 545
self-deﬁnition, 548ff
self-representation
own (static) variables, 127, 136
of undeclared variable in scripting
language, 739ff
scope resolution operator, 128, 477, 489;
...
scripting languages, 12, 699ff. See also
...
numbers, C 67
scope, 3, 116, 125ff. See also binding;
...
ﬁrst-class functions, 156, 431, 739
composite, 753ff
numeric, 752
declarations, 702
deﬁnition, 699
...
trampoline, 457
signaling NaN, ﬂoating-point, 239; C 70
...
implementation, 803
bottom-up
Smalltalk, 7, 12, 177, 472, 522ff, 669,
868; C 204ff
...
SPARC architecture, 633, 806; C 99ff,
437
stack pointer, 121, 412
stack smashing, 385
stack-based allocation and layout, 120ff,
C 105
addressing modes, C 72
atomic instructions, 655
condition codes, C 73
...
special forms in Scheme, 283, 292, 541,
412ff, 792. See also calling
...
stack-based intermediate language, 782,
523; C 204ff
parameter passing, 425
polymorphism, 302
precedence, 227
programming environment, 25ff, 41,
569, 695
speculation, 223
code caching, 823
code improvement, 184
811. See also Forth; bytecode;
Postscript
optimization, 812
stall of processor pipeline. See pipeline
523; C 148ff, C 204ff
OR parallelism, 686
processor execution, C 79, C 89,
self, 473; C 207
super, 480
...
subtype, 315, 320, 505. See also class,
vs expression, 224, 229, 233
in predicate calculus, 612; C 226
static analysis, 183ff. See also alias
in ML, 556
and scripting languages, 704, 712,
...
checking, strong
Stroustrup, Bjarne, 346, 530, 861; C 194,
derived
constrained, 310
subtype analysis, 184, 510
subtype polymorphism. See
C 197
structural equivalence of types, 313ff,
406, 560, 799
structural view of types, 300
structured programming, 7, 246ff, 295;
polymorphism, subtype
success, of computation in Icon, 305;
127, 136
static scoping. See scope, static
static semantics. See semantics, static
static single assignment (SSA) form, 825;
C 108
Sun Microsystems, Inc., 530, 736, 812,
C 324
goto alternatives, 247ff, 285
structures, 351ff. See also records
864. See also Oracle
Corporation
Sundell, Håkan, 696
...
stdio library, in C/C++, 436; C 154,
in Prolog, 593
stub
C 156
Stearns, Richard E., 112, 215
Steele, Guy L. Jr., 539, 862, 868
stop-and-copy garbage collection, 394ff,
...
subroutine, 223, 411ff. See also calling
C 355ff
Suraski, Zeev, 866
...
118ff. See also dangling
reference; garbage collection;
memory, leak
storage compaction, 123; C 146
store_conditional instruction, 657,
associative arrays, 359
line breaks, 48
mix-in inheritance, 516
nested subroutines, 127
...
higher-order
formal, 431. See also ﬁrst-class values,
in C++, C 156ff
in functional programming, 571ff,
subroutines
frame pointer, 120, 121, 412
generic. See generic subroutine or
586
...
synthesized
380
in object ﬁle, 36, 791, 829, 837ff
...
test in bash, 707
syntax, 3, 29, 43ff. See also context-free
...
phase, 27, 34ff
task, 635
Task Parallel Library (TPL) in .NET, 626,
scheduler-based
busy-wait (spinning), 636, 653ff, 665
bottom-up recovery, C 10ff
context-speciﬁc look-ahead, 102;
barriers, 655ff
and preemption, 693
...
639, 684
task parallelism, 643
Tcl (tool command language), 702, 869.
C 3ff, C 7
error productions, 103
misspelling, C 25
panic mode recovery, 102; C 1ff,
See also Tk
arrays and hashes, 755
context, use of, 703
error checking, 751
...
timing window, 651. See also race
temporal loop, in relaxed memory
model, 660ff
temporary ﬁle, 401; C 150
term, in Prolog, 593
terminal, of CFG, 49
...
Titanium, 698
Tk, 701, 869
tokens, 19, 28, 44ff. See also scanners
...
Thompson, Kenneth, 9, 306, 705, 744,
detection), 410; C 144ff, C 161
...
modules, 137
side effects, 242, 252
Turing Award, 536. See also entries for
top-down
topological sort, 618; C 330
Torczon, Linda, 41, 215, 806; C 106,
Fran Allen; John Backus; O.-J.
Dahl; Edsger Dijkstra; Tony
Hoare; Kenneth Iverson; Alan
Kay; Donald Knuth; Leslie
Lamport; Butler Lampson;
Barbara Liskov; John
McCarthy; Robin Milner;
Kristen Nygaard; Michael
Rabin; Dennis Ritchie; Dana
Scott; Ken Thompson; Alan
Turing; Niklaus Wirth
Turing, Alan, 536ff
863
Thompson shell, 705
...
co-begin, 638ff, 642
early reply, 646
implicit receipt, 646; C 244. See
...
typestate, 347
complex, 305
composite, 300, 310ff. See also arrays;
...
deﬁnition and point of view, 300ff
orthogonality, 302ff
polymorphism and, 302ff
purpose, 297
Urbana-Champaign, 418
...
virtual registers, 776; C 93, C 299,
languages, 706ff, 748ff
in logic programming, 592, 593,
C 307ff. See also register
allocation
...
variant records (unions), 238, 319, 351,
language-level, 305
machine-level, C 65
unstructured control ﬂow, 224, 246ff
unwinding of subroutine call stack
languages, 479, 488; C 27,
C 30
visitor pattern, 294, 514
Visual Basic, 861; C 286. See also Basic;
357ff; C 136ff
as composite type, 311
constrained, C 141
and garbage collection, 391
...
vector processor, 625, 640, 683; C 342ff,
W
...
Val (language), 11, 546
Valgrind, 176
value model of variables, 230ff, 377, 402,
Common Language
Infrastructure
VEST binary translator, 830, 854
viable preﬁx, of CFG, C 19
...
nops, 794
preﬁx code for instruction, C 86
register set, C 82ff
segmentation, 791
