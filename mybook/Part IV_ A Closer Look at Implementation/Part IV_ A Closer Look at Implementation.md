...
reg names : array [0..*k**−*1] of register name := [“r1”, “r2”, . . . , “r*k*”]
...
egy uses the next free reg inherited attribute to manage registers r1. . . r*k* as an
...
an address evenly divisible by 2*n*.
■
...
and ﬁelds. In the format shown here, “V” indicates void; “L*name*;” is a fully
...
tions in each block. If basic block* i* contains* k**i* instructions and executes* n**i* times
during a run of a program, it contributes* k**i**n**i* dynamic instructions to that run.
Let* N* = 
*i** **k**i**n**i* be the total number of instructions in the run. If statistical
sampling indicates that block* i* accounts for* x**i*% of the time in the run and* x**i* is
signiﬁcantly larger than (*k**i**n**i*)*/**N*, then something strange is going on—probably
