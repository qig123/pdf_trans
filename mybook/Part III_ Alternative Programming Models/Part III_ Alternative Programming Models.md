...
erwise known as* A-list*s). A-lists were introduced in Section C 3.4.2 in the context
...
cial forms are known as* macro*s in Scheme, but unlike most other macros, they
...
The* resolution principle*, due to Robinson [Rob65], says that if* C*1 and* C*2 are Horn
clauses and the head of* C*1 matches one of the terms in the body of* C*2, then we
can replace the term in* C*2 with the body of* C*1. Consider the following example.
**EXAMPLE** 12.8
...
tracking to simulate the execution of a* non*deterministic ﬁnite automaton.
12.5
...
is “level-*de*pendent”—it is speciﬁc to threads when implemented in the language
...
may suspend itself by* wait*ing on a condition variable. An operation may also
...
..., a*n*, and returns a future that will eventually yield f(a1, ... , a*n*):
...
(about 50 in bash), are* builtin*s—commands that the shell recognizes and ex-
...
tegers, sometimes known as* bignum*s. Python has bignums too, plus support for
